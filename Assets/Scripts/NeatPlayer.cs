...
public class NeatPlayer : GenericPlayer
{
    // Inputs:
    ...
    static readonly int numPlayers = Math.Min(Neat.FFA_size - 1, maxNumPlayers);
    ...
    public float fitness
    {
        get
        {
            return mnet.genoType.fitness;
        }
        set
        {
            mnet.genoType.fitness = value;
        }
    }
    // Outputs:
    //  left right (2)
    ...
    public static NeatPlayer MakeNeatPlayer()
    {
        Genome genome = new Genome(numInputs, numOutputs);

        return new NeatPlayer(new NEATNet(genome));
    }

    public NeatPlayer(NEATNet brain)
    {
    ...
    public NeatPlayer(NeuralNet neuralNet) : this((NEATNet)neuralNet)
    {
    }
    ...
    public NeatPlayer BreedPlayer(NeatPlayer otherParent)
    {
        return new NeatPlayer(mnet.Breed(otherParent.mnet));
    }
}
